...
if (isset($_POST['like_btn'])) {
    if ($user_id === '') {
        ...
        if ($check_like->rowCount() > 0) {
            ...
            // Fetch tutor_id for the content
...
if (isset($_POST['delete_comment'])) {
    $delete_id = filter_var($_POST['comment_id'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
...
if (isset($_POST['update_now'])) {
    $update_id = filter_var($_POST['update_id'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $update_box = filter_var($_POST['update_box'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    // If the new comment text is same as the old one, you may choose not to update
    $verify_comment = $conn->prepare("SELECT * FROM `comments` WHERE id = ? AND comment = ?");
    $verify_comment->execute([$update_id, $update_box]);
    if ($verify_comment->rowCount() > 0) {
        $message[] = 'Comment already updated!';
    } else {
...
<?php 
// Assume that user_header.php uses $message to display messages.
         ...
         if ($select_comments->rowCount() > 0) {
             while ($fetch_comment = $select_comments->fetch(PDO::FETCH_ASSOC)) {
                 ...
                 <div class="box" style="<?= ($fetch_comment['user_id'] == $user_id) ? 'order:-1;' : ''; ?>">
                     ...
                     <?php if (isset($_POST['edit_comment']) && $_POST['comment_id'] === $fetch_comment['id']): ?>
                        <section class='edit-comment'>
                            <form action="" method="post">
                                <input type="hidden" name="update_id" value="<?= $fetch_comment['id']; ?>">
                                <textarea name="update_box" class="box" maxlength="1000" required><?= $fetch_comment['comment']; ?></textarea>
                                <div class="flex-btn">
